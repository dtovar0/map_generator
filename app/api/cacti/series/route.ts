...
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as {
    range?: string; consolidation?: RrdConsolidation; series?: SeriesRequest[];
  } | null;
  if (!body || !Array.isArray(body.series) || body.series.length > 24 || !ranges.has(body.range || "24h") ||
      !consolidations.has(body.consolidation || "AVERAGE")) {
  ...
  try {
    ...
    const series = await Promise.all(valid.map(async (item) => {
      ...
      const points = source === "rrd"
        ? await getRrdSeries(item.localDataId, item.dsName, body.range, body.consolidation)
        : await getStoredSeries(item.localDataId, item.dsName, body.range, body.consolidation);
    ...
    return NextResponse.json({source, range:body.range || "24h", consolidation:body.consolidation || "AVERAGE", series});
  } catch (error) {
