...
| `AUTH_FORWARD_SECRET` | — | Valor esperado de `X-MapGen-Forward-Secret` |
...
**Interfaces:**
- Produces: tipo `Role = "admin" | "editor" | "viewer"`; `getAuthConfig(): AuthConfig` (campos: `localEnabled`, `forwardEnabled`, `forwardSecret`, `oidcEnabled`, `oidcIssuer`, `oidcClientId`, `oidcClientSecret`, `sessionSecret`, `sessionTtlHours`, `adminGroups: string[]`, `editorGroups: string[]`, `defaultRole: Role`, `bootstrapUser`, `bootstrapPassword`); `roleFromGroups(groups: string[], cfg): Role`; `hasRole(role: Role, min: Role): boolean`; `isRole(v: unknown): v is Role`.
...
export function getAuthConfig(): AuthConfig {
  ...
  return {
    ...
    forwardSecret: process.env.AUTH_FORWARD_SECRET || "",
...
- Produces:
  - `userFromForwardHeaders(request: Request): Promise<AppUser | null>` — exige header `x-mapgen-forward-secret === cfg.forwardSecret` (no vacío) y `remote-user`; cache en memoria de 60 s por username.
  - `FORWARD_SECRET_HEADER = "x-mapgen-forward-secret"`
...
export const FORWARD_SECRET_HEADER = "x-mapgen-forward-secret";
...
// proxied request, so no session row is created. The provisioned user is
// cached briefly to avoid a DB round-trip per request.
...
export async function userFromForwardHeaders(request: Request): Promise<AppUser | null> {
  ...
  if (!secretMatches(request.headers.get(FORWARD_SECRET_HEADER), cfg.forwardSecret)) return null;
...
**Interfaces:**
- Consumes: `requireUser` (T6), `SESSION_COOKIE` (T5), `FORWARD_SECRET_HEADER` (T6).
...
```ts
  ...
  if (!isPublicPage) {
    ...
    const forwardOk = process.env.AUTH_FORWARD_ENABLED === "true" &&
      Boolean(process.env.AUTH_FORWARD_SECRET) &&
      request.headers.get("x-mapgen-forward-secret") === process.env.AUTH_FORWARD_SECRET &&
...
```apache
# Apache + Authelia forward-auth (mod_auth_openidc no requerido)
<Location />
  AuthType None
  # ... configuración estándar de Authelia con mod_proxy y auth_request ...
  RequestHeader set X-MapGen-Forward-Secret "EL_MISMO_VALOR_DE_AUTH_FORWARD_SECRET"
  # Nunca dejar pasar estos headers desde el cliente:
  RequestHeader unset Remote-User "expr=-z %{reqenv:AUTHELIA}"
</Location>
```
...
# 4. Forward-auth (con AUTH_FORWARD_ENABLED=true y AUTH_FORWARD_SECRET=s3c):
curl -si localhost:3000/api/maps -H 'X-MapGen-Forward-Secret: s3c' \
